        ...
        while cap.isOpened():
            ...
            if result.multi_hand_landmarks:
                for i, hand_landmarks in enumerate(result.multi_hand_landmarks):
                    ...
                    if handedness == "Right":
                        gripper_closure = self.hand_closure_ratio(hand_landmarks.landmark)
                        gripper_closure = int(gripper_closure * 100)
                        if prev_closure:
                            gripper_closure = alpha_gripper*gripper_closure + (1-alpha_gripper)*prev_closure
                        ...
                        lm = hand_landmarks.landmark[self.mp_hands.HandLandmark.MIDDLE_FINGER_MCP]
                        pixel_x = int(lm.x * w)
                        pixel_y = int(lm.y * h)
                    ...
                    self.get_logger().info(f"x={robot_x:.2f}m, y={robot_y:.2f}m, closure={gripper_closure}, direction={direction}")
